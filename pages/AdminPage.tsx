...
const AdminPage: React.FC = () => {
  ...
  // Permanent Ledger Delete State
  const [showPermDeleteModal, setShowPermDeleteModal] = useState(false);
  const [ledgerToPermDelete, setLedgerToPermDelete] = useState<Cashbook | null>(null);
  ...
  const handlePermanentDeleteCashbook = async () => {
    if (!ledgerToPermDelete) return;
    setActionLoading(true);
    try {
      // Ensure we pass the ledger ID to hardDeleteCashbook
      await db.hardDeleteCashbook(ledgerToPermDelete.id);
      setLedgerToPermDelete(null);
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24">
      ...
      <ConfirmationModal 
        isOpen={showPermDeleteModal}
        onClose={() => setShowPermDeleteModal(false)}
        onConfirm={handlePermanentDeleteCashbook}
        title="Wipe Ledger Forever?"
        description={`Are you sure you want to permanently delete "${ledgerToPermDelete?.name}"? All transaction records and staff access will be removed permanently. This cannot be undone.`}
        isDanger={true}
              ...
              deletedBooks.map(cb => (
                <div key={cb.id} className="bg-white p-5 rounded-3xl border border-slate-100 flex items-center justify-between shadow-sm grayscale opacity-80">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setLedgerToPermDelete(cb); setShowPermDeleteModal(true); }} className="p-3 bg-rose-50 text-rose-600 rounded-2xl hover:bg-rose-100 active:scale-90 transition-all">
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                    </button>
                  </div>
                </div>
