...
type AdminView = 'menu' | 'add-category' | 'directory' | 'active-books' | 'recycle-bin' | 'add-staff';
    ...
    return (
      <div className="bg-slate-50 min-h-screen">
        ...
        <div className="p-6 grid grid-cols-2 gap-4">
          <button onClick={() => setCurrentView('directory')} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center active:scale-95 transition-all">
          ...
          <button onClick={() => setCurrentView('add-staff')} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center active:scale-95 transition-all">
          ...
          <button onClick={() => setCurrentView('active-books')} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center active:scale-95 transition-all">
          ...
          <button onClick={() => setCurrentView('add-category')} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center active:scale-95 transition-all">
          ...
          <button onClick={() => setCurrentView('recycle-bin')} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center active:scale-95 transition-all col-span-2">
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24">
      <div className="bg-blue-600 text-white pt-12 pb-6 px-6 shadow-lg flex items-center gap-3 sticky top-0 z-[100]">
        <button onClick={handleBack} className="w-8 h-8 flex items-center justify-center hover:bg-white/10 rounded-full">
          <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
        </button>
        ...
        {currentView === 'add-staff' && (
          <form onSubmit={handleAddStaff} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-5 animate-in slide-in-from-bottom-4 duration-300">
             ...
             <div>
                ...
                <input type="email" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold outline-none focus:bg-white focus:border-blue-600 transition-all" value={newStaffEmail} onChange={e => setNewStaffEmail(e.target.value)} required placeholder="staff@business.com" />
             </div>
             <div>
             ...
             <button type="submit" disabled={loading} className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl shadow-xl shadow-blue-100 active:scale-95 transition-all mt-4 uppercase text-xs tracking-[0.2em]">
          ...
          <>
            ...
            {users.filter(u => u.role !== UserRole.OWNER).map(u => (
              <div key={u.id} className="bg-white p-4 rounded-3xl border border-slate-100 flex items-center justify-between shadow-sm">
                ...
                <button onClick={() => openEditUser(u)} className="p-3 bg-slate-50 text-blue-600 rounded-2xl hover:bg-blue-50 transition-colors">
          ...
          <>
            ...
            {deletedBooks.length === 0 ? (
              <div className="py-20 text-center">
                 <div className="text-slate-300 mb-2">Trash is empty</div>
              </div>
            ) : (
              deletedBooks.map(cb => (
      ...
      {showStaffModal && managingCb && (
        <div className="fixed inset-0 z-[150] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl flex flex-col max-h-[80vh] overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto space-y-4 no-scrollbar">
              ...
              {assignedStaff.length === 0 && (
                <div className="py-12 text-center">
                  <div className="text-slate-300 mb-2">No staff assigned</div>
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Assign staff to this book from its settings</div>
                </div>
              )}
            </div>
          </div>
